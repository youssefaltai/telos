...
> This document captures the *reasoning* behind Telos's design decisions.
...
Without the Prompt Engineer, the system is static. Every run is as good as the first. With it, every run is a feedback signal that makes the next run better. This is the mechanism that makes Telos genuinely different from a pipeline — it's a system that compounds.
